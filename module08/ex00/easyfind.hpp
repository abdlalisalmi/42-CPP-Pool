...
/*   Updated: 2022/03/20 11:17:18 by aes-salm         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int easyFind(T vec, int occurrence) { // for handle Vectors and Arrays
	...
	if (iter == vec.end()) throw std::runtime_error("Occurrence not found!");
...
template <typename T>
int easyFind(std::priority_queue<T> queue, int occurrence) { // for handle priority queues First-In First-Out
	std::vector<T> vec;
	size_t queue_len = queue.size();
	for (size_t i = 0; i < queue_len; i++)
	{
